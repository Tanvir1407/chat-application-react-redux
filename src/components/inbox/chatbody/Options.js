...
import { useEditConversationMutation } from "../../../features/conversions/conversionsApi";
...
export default function Options({info}) {
    ...
    const participantsUser = info.sender.email === user.email ? info.receiver : info.sender;
    ...
    const handleSubmit = (e) => {
        ...
        editConversation({
            ...
            sender: user.email,
            data: {
              participants: `${user.email}-${participantsUser.email}`,
              users: [user,participantsUser],
              message,
